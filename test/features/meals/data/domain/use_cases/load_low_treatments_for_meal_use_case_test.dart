...
import 'package:diabeatthis/features/meals/data/domain/use_cases/load_low_treatments_for_meal_use_case.dart';
...
void main() {
  ...
  test('loads low treatments related to meal', () async {
    final relatedMeal = await db
        .into(db.meal)
        .insertReturning(
          MealCompanion.insert(
            name: 'Obiad',
            plannedAt: DateTime(2026, 5, 24, 12).millisecondsSinceEpoch,
            status: const Value('summarized'),
          ),
        );
    final lowTreatmentAt = DateTime(2026, 5, 24, 13);
    ...
    final treatments = await container.read(
      mealLowTreatmentsForMealUseCaseProvider(relatedMeal.id).future,
    );

    expect(treatments, hasLength(1));
    expect(treatments.single.meal.id, lowTreatment.id);
    expect(treatments.single.context.relatedMealId, relatedMeal.id);
